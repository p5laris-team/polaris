...
class ItemEventLogEventListenerTest {
    ...
    @InjectMocks
    private ItemEventLogEventListener itemEventLogEventListener;

    @Test
    @DisplayName("handle - 이벤트 수신 시 정상적으로 Outbox 테이블에 saveAndFlush 호출")
    void handle_success() throws Exception {
        // given
        Item item = Item.builder().id(10L).name("테스트 스킨").itemType("SKIN").price(100).build();
        UserItem userItem = UserItem.builder().id(50L).userId(1L).item(item).quantity(1).build();
        
        ItemEventLogEvent event = ItemEventLogEvent.itemPurchased(
                1L, userItem, item, 1, 100, 12345L, 400
        );

        when(objectMapper.writeValueAsString(any())).thenReturn("{\"mock\":\"payload\"}");

        // when
        itemEventLogEventListener.handle(event);
    ...
    void handle_exception_suppressed() throws Exception {
        ...
        // when & then
        assertThatNoException().isThrownBy(() -> itemEventLogEventListener.handle(event));
